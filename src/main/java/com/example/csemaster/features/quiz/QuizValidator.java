...
import com.example.csemaster.features.ListUtils;
...
public class QuizValidator {
    ...
    public boolean isValidDetailSubject(String subject, List<String> detailSubject) {
        try {
            ...
            List<ChapterEntity> bass = getSubjectEntity(subject).getDetailSubjects();
            // db 의 내용과 요청한 내용과 동일한지 비교
            return ListUtils.compareList(bass.stream().map(ChapterEntity::getDetailSubject).toList(), detailSubject);
        } catch (Exception e) {
